...
- **(B) 얇은 provider 추상화 (B-thin)** — `LLMClient` Protocol 1개 + Gemini 참조 구현 1개. 다른 provider (Anthropic / 사내 LLM / vLLM 등) 의 실제 SDK 시그니처는 본 PRD 에 박지 않는다. 도메인이 자기 어댑터를 등록하는 슬롯만 노출.
...
1. **Anthropic 어댑터 본 구현** — `LLMClient` Protocol 적합 슬롯만 열어둠. 본 구현은 도메인 또는 후속 Phase 책임.
